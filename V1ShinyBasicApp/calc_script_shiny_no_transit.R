...
myelectricity_calculate <- function(home_type, current_status, upgrade_status, elec,
                                    ...
                                    elec_increase, nat_gas_increase) {
  ...
  combined_sums$Month <- factor(combined_sums$monthly_sums.Month, levels = month_order)
  
  # Create the stacked bar chart
  breakdown <- ggplot(combined_sums, aes(x = Month, y = count, fill = type)) +
  ...
  # Print the results
  start_xx <- paste0("Total Start Cost:", total_start_cost, "\n")
  upgrade_xx <- paste0("Total Upgrade Cost:", total_upgrade_cost, "\n")
  savings_xx <- paste0("Difference (Upgrade - Start):", difference, "\n")
  ...
  result <- list(plot1 = breakdown, plot2 = bar_plot)
  return(result)
}
